...
import { RISK_FACTORS, calculateBleedingRisk, calculateIschemicRisk, determineRiskZone, TRADE_OFF_SLOPES } from './risk-factors.js';
...
function generateHTML() {
    ...
    const toRiskFactorItem = (factor, type = 'checkbox', groupName, isDefault = false) => uiBuilder.createRiskFactorItem({
        id: `factor-${factor.id}`,
        label: factor.label,
        type,
        name: groupName,
...
function getSelectedFactorIds() {
    const checkboxes = document.querySelectorAll('[data-factor-id]:checked');
...
function updateCalculation() {
    ...
    if (bleedingEl)
        bleedingEl.textContent = `${bleedingRisk.toFixed(1)}%`;
    if (ischemicEl)
        ischemicEl.textContent = `${ischemicRisk.toFixed(1)}%`;
    ...
    if (resultContainer) {
        resultContainer.innerHTML = `
            <div class="result-zone ${zoneResult.zone}">
                <strong>${zoneResult.zone === 'ischemic_dominant'
            ? '💔 Ischemic Dominant'
...
async function autoPopulate() {
    try {
        ...
        if (age !== null) {
            if (age >= 65) {
                const ageCheckbox = document.getElementById('factor-age_65');
                if (ageCheckbox) {
        ...
        for (const condition of conditions) {
            ...
            if (factorId) {
                const checkbox = document.getElementById(`factor-${factorId}`);
                if (checkbox) {
        ...
        if (hbObs?.value) {
            ...
            if (hb < 11) {
                const checkbox = document.getElementById('factor-hb_lt_11');
                if (checkbox)
                    checkbox.checked = true;
            }
            else if (hb < 13) {
                const checkbox = document.getElementById('factor-hb_11_13');
                if (checkbox)
                    checkbox.checked = true;
            }
        }
        if (egfrObs?.value) {
            ...
            if (egfr < 30) {
                const checkbox = document.getElementById('factor-egfr_lt_30');
                if (checkbox)
                    checkbox.checked = true;
            }
            else if (egfr < 60) {
                const checkbox = document.getElementById('factor-egfr_30_59');
                if (checkbox)
                    checkbox.checked = true;
            }
        }
