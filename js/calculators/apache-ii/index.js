...
export const apacheIi = createUnifiedFormulaCalculator({
    ...
    customInitialize: (client, patient, container) => {
        ...
        const sbpInput = container.querySelector('#apache-ii-sbp');
        const dbpInput = container.querySelector('#apache-ii-dbp');
        const mapInput = container.querySelector('#apache-ii-map');
        const updateMap = () => {
        ...
        const paco2Input = container.querySelector('#apache-ii-paco2');
        // Find the group (container for label + input wrapper) to hide everything
        const paco2Group = paco2Input?.closest('.ui-input-group');
        const updatePaco2Visibility = () => {
            const method = container.querySelector('input[name="oxy_method"]:checked');
