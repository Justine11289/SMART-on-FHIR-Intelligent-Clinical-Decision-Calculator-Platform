import { cdcData, mapLmsToPercentileData } from './cdc-data.js';
import { calculateZScore, estimatePercentile, calculateBmiData, calculateVelocity } from './calculation.js';
...
export const growthChart = {
    ...
    initialize: function (client, patient, container) {
        ...
        function updateChartStatus(type, dataCount, latestValue, latestAge, cdcDataArray) {
            ...
            try {
                ...
                const processObservations = (observations) => {
                    return observations
                        ...
                        .map(obs => ({
                        ageMonths: (new Date(obs.effectiveDateTime).getTime() - birthDate.getTime()) /
        ...
        getGrowthData().then(data => {
            if (data) {
                ...
                if (heightCanvas) {
                    createChart(heightCanvas, 'Height for Age', filteredHeight, cdcHeightData, 'Height (cm)', 'Patient Height');
                    updateChartStatus('height', filteredHeight.length, latestHeight ? latestHeight.value : null, latestHeight ? latestHeight.ageMonths : null, cdcHeightData);
                }
                ...
                if (weightCanvas) {
                    createChart(weightCanvas, 'Weight for Age', filteredWeight, cdcWeightData, 'Weight (kg)', 'Patient Weight');
                    updateChartStatus('weight', filteredWeight.length, latestWeight ? latestWeight.value : null, latestWeight ? latestWeight.ageMonths : null, cdcWeightData);
                }
                ...
                if (bmiCanvas) {
                    createChart(bmiCanvas, 'BMI for Age', filteredBMI, cdcBmiData, 'BMI (kg/m²)', 'Patient BMI');
                    updateChartStatus('bmi', filteredBMI.length, latestBMI ? latestBMI.value : null, latestBMI ? latestBMI.ageMonths : null, cdcBmiData);
                }
