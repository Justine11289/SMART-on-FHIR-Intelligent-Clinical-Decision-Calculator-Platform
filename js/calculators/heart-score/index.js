...
export const heartScoreConfig = {
    ...
    customInitialize: async (client, patient, container) => {
        ...
        try {
            ...
            let riskCount = 0;
            // HTN
            if (await fhirDataService.hasCondition([SNOMED_CODES.HYPERTENSION]))
                riskCount++;
            // Hyperlipidemia
            if (await fhirDataService.hasCondition([SNOMED_CODES.HYPERLIPIDEMIA]))
                riskCount++;
            // Diabetes
            if (await fhirDataService.hasCondition([
            ...
            if (await fhirDataService.hasCondition([
                ...
                riskCount++;
            // Family History
            if (await fhirDataService.hasCondition([SNOMED_CODES.FAMILY_HISTORY_CAD]))
                riskCount++;
            // Obesity (BMI > 30)
            const bmi = await fhirDataService.getObservation(LOINC_CODES.BMI);
            const isObese = (bmi.value !== null && bmi.value > 30) ||
