...
export const ascvd = createUnifiedFormulaCalculator({
    ...
    formulaSection: {
        ...
        calculationNote: 'Risk = 1 - S₀^exp(IndividualSum - MeanCoefficient), where S₀ is baseline survival',
        scoringCriteria: [
    ...
    customInitialize: (client, patient, container, calculateFn) => {
        ...
        const baselineDisplay = container.querySelector('#therapy-baseline-risk');
        ...
        if (calcBtn) {
            calcBtn.addEventListener('click', () => {
                const resultsEl = container.querySelector('#therapy-results');
                if (!resultsEl || !currentPatientData)
                    return;
                ...
                const interventions = [];
                // Apply Statin Effect
                const statinInput = container.querySelector('#statin-therapy');
                ...
                const lifestyleInput = container.querySelector('#lifestyle-mods');
                if (lifestyleInput?.checked) {
                    const stopSmoke = container.querySelector('#smoking-cessation');
                    if (stopSmoke?.checked && modPatient.isSmoker) {
                        modPatient.isSmoker = false;
                        interventions.push('Smoking cessation');
                    }
                    const bpControl = container.querySelector('#bp-control');
                ...
                const addInput = container.querySelector('#additional-therapy');
                if (addInput?.checked) {
                    const opt = container.querySelector('#additional-options').value;
